...
%This is version 1.0 (Last edited: 2017-11-04) License: This code is licensed under the GPLv2 license. If you in any way
%use this code for research that results in publications, please cite our monograph as described above.

% Generate channel realizations. Generate uncorrelated Rayleigh fading channel realizations
...
for j = 1:L    
    ...
    for g = 1:f        
        ...
        for k = 1:K            
            ...
            for l = groupMembers                
                %Compute MMSE estimate of channel between BS l and UE k in cell j using (3.9) in Theorem 3.1
                RPsi = R(:,:,k,l,j) / PsiInv;
                ...
                %If EW-MMSE estimation is to be computed
                if nargout >= 5                    
                    %Compute EW-MMSE estimate of channel between BS l and UE k in cell j using (3.33)
                    A_EW_MMSE = diag(sqrt(p)*diag(R(:,:,k,l,j)) ./ PsiInvDiag);
                    Hhat_EW_MMSE(:,:,k,l,j) = A_EW_MMSE*yp(:,:,k);                    
                    %Compute corresponding estimation error correlation matrix, using the principle from (3.29)
                ...
                if nargout >= 7                    
                    %Compute LS estimate of channel between BS l and UE k in cell j using (3.35) and (3.36)
                    A_LS = 1/(sqrt(p)*tau_p);
