...
%Compute UL SE for different receive combining schemes with hardware impairments, based on Theorem 6.2.
%% INPUT:
...
%This Matlab function was developed to generate simulation results to: Emil Bjornson, Jakob Hoydis and Luca Sanguinetti (2017), 
%"Massive MIMO Networks: Spectral, Energy, and Hardware Efficiency", Foundations and Trends in Signal Processing: Vol. 11, No. 3-4, 
%pp. 154-655. DOI: 10.1561/2000000093.

%Store identity matrices of different sizes
eyeK = eye(K);eyeM = eye(M);
...
signal_MR = zeros(K,L); signal_RZF = zeros(K,L); signal_MMMSE = zeros(K,L);

%Prepare to store simulation results for norms of combining vectors
combiningNorm_MR = zeros(K,L); combiningNorm_RZF = zeros(K,L); combiningNorm_MMMSE = zeros(K,L);

%Prepare to store simulation results for sum interference powers 
interf_MR = zeros(K,L); interf_RZF = zeros(K,L); interf_MMMSE = zeros(K,L);
%Prepare to store simulation results for sum impairment-caused interference
impair_MR = zeros(K,L); impair_RZF = zeros(K,L); impair_MMMSE = zeros(K,L);
    ...
    for j = 1:L        
        ...
        for k = 1:K            
            ...
            if norm(V_MR(:,k))>0                
                ...
                wrep = repmat(w,[1 K*L]);                
                %Compute realizations of the expectations in signal and interference terms in (6.32)
...
sigma2 = 1/(kappatUE*kapparBS); factor1 = 1/kappatUE; factor2 = (1-kapparBS)/(kappatUE*kapparBS);
