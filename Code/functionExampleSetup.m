...
UEpositions = zeros(K,L); perBS = zeros(L,1);
...
for l = 1:L    
    ...
    %Go through all BSs
    for j = 1:L        
        %Compute the distance from the UEs in cell l to BS j with a wrap around topology, where the shortest distance between a UE and the
        ...
        %Compute average channel gain using the large-scale fading model in (2.3), while neglecting the shadow fading 
        channelGaindB(:,l,j) = constantTerm - alpha*10*log10(distancesBSj);        
        %Compute nominal angles between UE k in cell l and BS j, and generate spatial correlation matrices for the channels using the
    ...
    for k = 1:K        
        ...
        %Check if another BS has a larger average channel gain to the UE than BS l
        while channelGainShadowing(l) < max(channelGainShadowing)            
            %Generate new shadow fading realizations (until all UEs in cell l has its largest average channel gain from BS l)
            shadowing = sigma_sf*randn(1,1,L);
            channelGainShadowing = channelGaindB(k,l,:) + shadowing;            
        end        
