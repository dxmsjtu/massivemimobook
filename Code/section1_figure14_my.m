...
numberOfRealizations = 10000; % 100000
...
for k = 1:numberOfRealizations    
    ...
    %implemented slightly differently from (1.28) using the identity sin^2(x)= (1-cos(2x))/2   
    ...
    SE_NLoS_montecarlo(:,k) = log2(1 + SNR*cumsum(abs(fadingRealizationsDesired(:,k)).^2) ./...,  %  using (1.32)
        (betabar*SNR*(abs(cumsum(conj(fadingRealizationsDesired(:,k)).*fadingRealizationsInterfering(:,k))).^2)./cumsum(abs(fadingRealizationsDesired(:,k)).^2) +1 ));    
end
...
legend('LoS','NLoS: Rayleigh fading montecarlo','NLoS: Rayleigh fading (lower bound)','Location','SouthEast');
Post_plot;