...
%License: This code is licensed under the GPLv2 license. If you in any way use this code for research that
...
SE_MMSE_NLoS_montecarlo = zeros(length(K),length(c)); SE_MMSE_LoS_montecarlo = zeros(length(K),length(c));
...
for n = 1:numberOfRealizations    
    ...
    for kindex = 1:length(K)        
        ...
        for cindex = 1:length(c)            
            ...
            M = K(kindex)*c(cindex);
            %Compute the SE with M-MMSE under LoS propagation for one realization of the UE angles            
...
xlabel('Number of UEs (K)'); ylabel('Average sum SE [bit/s/Hz/cell]');
legend('LoS','NLoS','Location','NorthWest'); ylim([0 120]);
Post_plot;