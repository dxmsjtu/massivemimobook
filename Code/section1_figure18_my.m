...
%% For further information, visit: https://www.massivemimobook.com
%This is version 1.01 (Last edited: 2020-04-21) License: This code is licensed under the GPLv2 license. If you in any way use this code for research that
...
SE_MMSE_NLoS_montecarlo = zeros(length(K),length(c)); SE_MMSE_NLoS_nonlinear = zeros(length(K),length(c));
...
for n = 1:numberOfRealizations    
   ...
   if mod(n,10)==1  disp([num2str(n) ' realizations out of ' num2str(numberOfRealizations)]);     end  
    ...
    for kindex = 1:length(K)        
        ...
        for cindex = 1:length(c)            
            ...
            SE_MMSE_NLoS_montecarlo(kindex,cindex) = SE_MMSE_NLoS_montecarlo(kindex,cindex) +  ...,
                sum(log2(1 + signalpowers./(interferencepowers+scalednoisepower)))/numberOfRealizations;            
        end
    end
