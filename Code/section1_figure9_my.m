...
%monograph as described above. %Empty workspace and close figures
close all; clear all;
%Define the SNR range for analytical curves
SNRdB = -10:0.1:30; SNR = 10.^(SNRdB/10);
...
numberOfFadingRealizations = 1000; %100000;
...
for b = 1:length(betabar)
    ...
    plot(SNRdB,SE_LoS(b,:),'k-','LineWidth',LineWidth);
    plot(SNRdB_montecarlo(1),SE_NLoS_montecarlo(1),'bd-.','LineWidth',LineWidth);
    plot(SNRdB_montecarlo,SE_NLoS_montecarlo,'bd','LineWidth',LineWidth);
    plot(SNRdB,SE_NLoS(b,:),'b-.','LineWidth',LineWidth);
