...
for r = 1:length(varphiRange)    
    ...
    for s = 1:numberOfRealizations        
        ...
        %Compute the NMSE in (3.10) (3.11), and  (3.20) if the correlation matrix is known
        NMSE_ideal(1,r) = real(trace(R - SNR*R*((SNR*R+eye(M))\R)))/trace(R);
        %Go through the different number of samples in the correlation estimates
        for n = 1:length(sampleRange)            
            ...
            %Go through all different linear combinations of the sample correlation matrix and the diagonalized version of it
            for c = 1:length(linearCombination)                
                ...
                %Compute the matrix that the received pilot signal is multiplied with in the approximate MMSE estimator
                A = sqrt(SNR)*Rest/(SNR*Rest+eye(M));               
                %Compute the NMSE in (3.20) (3.29) with this approximate estimator
