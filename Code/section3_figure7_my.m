...
for n1 = 1:length(varphiDesiredRadians)
    ...
    if mod(n1,10) == 1 disp([num2str(n1) ' angles out of ' num2str(length(varphiDesiredRadians))]);    end    
    ...
    for n2 = 1:length(varphiInterfererRadians)
        ...
        for s = 1:length(SNR1dB)            
            %Compute the NMSE in  (3.10)  (3.11) (3.20) with the MMSE estimator
            NMSE_MMSE(s,n1,n2) = real(trace(R1 - SNR1(s)*R1*((SNR1(s)*R1+SNR2(s)*R2+eye(M))\R1)))/trace(R1);                        
            %Compute the matrix in (3.33) that the received pilot signal is multiplied with in the EW-MMSE estimator
            A_EWMMSE = (sqrt(SNR1(s))/(SNR1(s)+SNR2(s)+1))*eye(M);            
            %Compute the NMSE in (3.20) (3.29) with the EW-MMSE estimator
            ...
            %Compute the matrix in (3.36) that the received pilot signal is multiplied with in the LS estimator
