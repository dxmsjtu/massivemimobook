%% This Matlab script can be used to reproduce Figures 4.14 in the monograph:
...
for n = 1:nbrOfSetups    
    ...
    for s = 1:length(fRange)        
        ...
        f = fRange(s);        
        %Generate channel realizations with estimates and estimation error correlation matrices, using MMSE, EW-MMSE, and LS estimators
        ...
        sumSE_MR(s,1,n) = mean(sum(SE_MR,1));        sumSE_RZF(s,1,n) = mean(sum(SE_RZF,1));        sumSE_MMMSE(s,1,n) = mean(sum(SE_MMMSE,1));     
        ...
        sumSE_MR(s,2,n) = mean(sum(SE_MR,1));        sumSE_RZF(s,2,n) = mean(sum(SE_RZF,1));        sumSE_MMMSE(s,2,n) = mean(sum(SE_MMMSE,1));     
        ...
        sumSE_MR(s,3,n) = mean(sum(SE_MR,1));       sumSE_RZF(s,3,n) = mean(sum(SE_RZF,1));       sumSE_MMMSE(s,3,n) = mean(sum(SE_MMMSE,1));        
