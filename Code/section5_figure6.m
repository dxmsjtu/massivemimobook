%This Matlab script can be used to reproduce Figure 5.6 in the monograph:
%
...
maxEE_teo = zeros(length(Mrange),1);
maxSE_teo = zeros(length(Mrange),1);
...
for index1 = 1:length(Mrange)
    ...
    for index2 = 1:length(SE)
        ...
        %Compute EE using (5.23)
        EE(index2,index1) = B*SE(index2)/(p/mu + P_FIX + M*P_BS);
