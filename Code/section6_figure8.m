%This Matlab script can be used to reproduce Figure 6.8 in the monograph:
%
...
for n = 1:nbrOfSetups
    ...
    for r = 1:length(kappatUE)
        ...
        for m = 1:length(Mrange)
            ...
            %Compute SE using Corollary 6.6
...
for r = 1:length(kappatUE)
    ...
    plot(Mrange,max(mean(sumSE_MR_asymptotics(:,r,:),3),[],2),'r--','LineWidth',1);
    
end

xlabel('Number of antennas (M)');
ylabel('Average sum SE [bit/s/Hz/cell]');
legend('MR','Asymptotic limit','Location','NorthWest');
